...
<template>
  <div class="w-full pt-20 pb-12 flex flex-col gap-10 select-none overflow-x-hidden">
    ...
    <MediaRow
      ...
      :title="`Reprendre avec le profil de ${firstName}`"
    ...
    <div class="flex flex-col gap-10">
      ...
      <Top10Row
        v-if="top10France.length > 0 || recentAdditions.length > 0"
        title="Top 10 en France aujourd'hui"
        :items="top10France.length > 0 ? top10France : recentAdditions"
        :is-loading="isLoading && top10France.length === 0 && recentAdditions.length === 0"
      />

      <!-- Section 3: Films (Supabase Movies) -->
      <MediaRow
        v-if="moviesRail.length > 0"
        title="Films"
        :items="moviesRail"
      />

      <!-- Section 4: Séries (Supabase TV) -->
      <MediaRow
        v-if="seriesRail.length > 0"
        title="Séries"
        :items="seriesRail"
      />

      <!-- Sections 5+: One rail per real genre actually present in the catalog -->
      <template v-for="genreRow in genreRails" :key="genreRow.genre">
        <MediaRow
          v-if="genreRow.items.length > 0"
          :title="genreRow.genre"
          :items="genreRow.items"
        />
      </template>

      <!-- Last Section: Chefs-d'œuvre & Mieux Notés (★) -->
      <MediaRow
        v-if="topRatedRail.length > 0"
