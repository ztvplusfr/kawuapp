...
// Textless poster in-memory cache
const posterCache = new Map()
// Shared /images response cache (one request serves both logo & textless poster)
const imagesResponseCache = new Map()

export function getTmdbApiKey() {
  if (typeof localStorage !== 'undefined') {
    return localStorage.getItem('kawu_tmdb_key') || USER_TMDB_API_KEY
  }
  return USER_TMDB_API_KEY
}
...
export function getTmdbLogoImage(path, size = 'original') {
  if (!path) return null
  if (path.startsWith('http')) return path
  return `${TMDB_IMAGE_BASE}/${size}${path}`
}
 ...
 */
async function fetchMediaImages(id, type) {
  const cacheKey = `${type}_${id}`
  if (imagesResponseCache.has(cacheKey)) return imagesResponseCache.get(cacheKey)
  ...
  const promise = (async () => {
    try {
      ...
      console.warn(`Failed to fetch images for ${type} ${id}:`, err)
      return null
    }
  })()

  imagesResponseCache.set(cacheKey, promise)
  return promise
}
...
export async function fetchLogoForMedia(id, type = 'movie') {
  ...
  const data = await fetchMediaImages(id, type)
...
/**
 * Fetch a textless poster (no title/language text baked into the artwork).
 * Falls back to null if TMDB has no textless variant for this title.
 */
export async function fetchTextlessPoster(id, type = 'movie') {
  const cacheKey = `${type}_${id}`
  if (posterCache.has(cacheKey)) return posterCache.get(cacheKey)

  const data = await fetchMediaImages(id, type)
  if (data && data.posters && data.posters.length > 0) {
    const textless = data.posters.find(p => p.iso_639_1 === null)
    if (textless) {
      const url = getTmdbImage(textless.file_path, 'original')
...
export function formatMediaItem(item, customCategory = null, logoUrl = null) {
  ...
  return {
    ...
    poster: getOriginalTmdbImage(item.backdrop_path || item.poster_path),
    posterUrl: getOriginalTmdbImage(item.poster_path || item.backdrop_path),
...
export async function getMediaFullDetails(id, type = 'movie') {
  ...
  try {
    ...
    const realActors = (data.credits?.cast || []).slice(0, 16).map(actor => ({
      ...
      photo: actor.profile_path ? getOriginalTmdbImage(actor.profile_path) : null
    }))
  ...
  if (seasonObj && seasonObj.episodes && seasonObj.episodes.length > 0) {
    return seasonObj.episodes.map(ep => {
      ...
      return {
        ...
        still: ep.still_path ? getOriginalTmdbImage(ep.still_path) : null,
...
/**
 * 10. Calendrier Réel Multi-Catégories (Animés, Séries, Films) via TMDB
 * Récupère les sorties programmées pour une plage de dates donnée.
 */
export async function fetchCalendarAiringMedia({ startDate, endDate, type = 'all', page = 1 }) {
  try {
    const params = {
      page: String(page)
    }
    ...
    if (type === 'anime') {
      ...
      params['air_date.gte'] = startDate
      params['air_date.lte'] = endDate
      params['sort_by'] = 'popularity.desc'
      const data = await fetchFromTmdb('/discover/tv', params)
      return (data?.results || []).map(item => ({ ...item, media_type: 'tv', calendarCategory: 'anime' }))
    }
    ...
    if (type === 'tv') {
      ...
      params['sort_by'] = 'popularity.desc'
      const data = await fetchFromTmdb('/discover/tv', params)
      return (data?.results || []).map(item => ({ ...item, media_type: 'tv', calendarCategory: 'tv' }))
    }
    ...
    if (type === 'movie') {
      ...
      return (data?.results || []).map(item => ({ ...item, media_type: 'movie', calendarCategory: 'movie' }))
    }

    // Si 'all' : paralléliser animés, séries TV et films pour la période
    const [animeRes, tvRes, movieRes] = await Promise.all([
      fetchCalendarAiringMedia({ startDate, endDate, type: 'anime', page }),
      fetchCalendarAiringMedia({ startDate, endDate, type: 'tv', page }),
      fetchCalendarAiringMedia({ startDate, endDate, type: 'movie', page })
    ])

    return [...animeRes, ...tvRes, ...movieRes]
  } catch (err) {
    console.warn('[TMDB Calendar] Error fetching calendar airing media:', err)
    return []
  }
}
