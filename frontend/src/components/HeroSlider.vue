...
const fallbackSlides = [
  {
    ...
    quality: '4K Ultra HD',
    rating: '★ 8.2',
    synopsis: 'Dans une mégalopole contrôlée par des méga-corporations, un pirate dissident s\'infiltre au cœur du réseau quantique.',
    bgImg: 'https://images.unsplash.com/photo-1578632767115-351597cf2477?q=80&w=1600&auto=format&fit=crop',
...
const activeSlides = computed(() => {
  return heroSlides.value && heroSlides.value.length > 0 ? heroSlides.value : fallbackSlides
})
...
<template>
  ...
  >
    <CarouselContent class="-ml-0">
      <CarouselItem v-for="slide in activeSlides" :key="slide.id" class="pl-0">
        
        <!-- Individual Cinema Hero Card -->
        <div class="relative w-full h-[500px] sm:h-[560px] lg:h-[600px] rounded-3xl overflow-hidden border border-cyan-500/20 shadow-2xl shadow-black/95 flex flex-col justify-between p-7 sm:p-12 select-none">
          ...
          <div class="relative z-20 flex flex-col sm:flex-row sm:items-end justify-between gap-6 w-full">
            ...
            <div class="flex flex-col gap-3.5 max-w-2xl">
              ...
              <div class="flex items-center gap-2 text-xs font-semibold text-white/80 flex-wrap">
                ...
                <span class="text-cyan-300">{{ slide.genre }}</span>
                <span v-if="slide.year" class="text-white/40">•</span>
                <span v-if="slide.year">{{ slide.year }}</span>
                <span class="px-1.5 py-0.5 rounded bg-white/15 border border-white/20 text-[10px] font-bold text-white">
                  {{ slide.age }}
                </span>
